...
class Temporal_Brain_Layer(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        if return_attention:
            ...
            # --- THE FIX: The CEO Ego Check ---
            # Slice the Keys (k) to ONLY include the 25 physical joints before the math!
            physical_k = k[:, :, :25, :] 
            
            # Compute raw scores only against the 25 joints
            spatial_scores = torch.matmul(global_q, physical_k.transpose(-2, -1)) / math.sqrt(q.size(-1))
